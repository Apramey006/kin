...
import { responseJSON } from "@/lib/client-auth";
...
export default function Stage() {
  ...
  const pendingReplay = useRef<AbortController | null>(null);
  useEffect(() => {
    audio.current = new Audio(SILENT_AUDIO);
    return () => {
      pendingReplay.current?.abort();
  ...
  const source = evidence ? {memoryId:evidence.memoryId, quote:evidence.supportedFacts.join(" "),
    contributorName:data?.relatives.find(r=>r.id===evidence.contributorId)?.name ?? "Your family"} : null;
  const question = data?.questions.find((q) => q.status === "open");
  const target = data?.relatives.find(
    (r) => r.id === question?.target_relative_id,
  );
  const replay = async () => {
    if (pendingReplay.current) return;
    const controller = new AbortController();
    pendingReplay.current = controller;
    unlockAudio(audio.current);
    setBusy("replay");
    setActionError(null);
    try {
      const p = await responseJSON(await fetch("/api/recall", { signal: controller.signal }));
      if (!p.lastEventId)
              ...
              <section className="current-moment" aria-live="polite">
                ...
                {!event ? (
                  <>
                    ...
                    <p className="caption">
                      Recognition needs a labeled photo and supporting memories from two people.
                    </p>
                  </>
                  ...
                  <>
                    ...
                    <p>
                      ...
                      family’s memories provide enough evidence.
                    </p>
                    {hasKnown && (
                      <Button
                        variant="outline"
                  ...
                  {data.relatives.map((r) => {
                    ...
                    return (
                      <details className="source-detail" key={r.id}>
                        ...
                        {k?.evidence?.length ? (
                          k.evidence.map((m) => (
                            <p className="source-evidence" key={m.memoryId}>
                              {m.supportedFacts.join(" ")}
                            </p>
                          ))
                        ) : (
                          <p className="caption">
