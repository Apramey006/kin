  ...
  try {
    ...
    const [
      ...
      sb
        .from("memories")
        .select(
          "id,family_id,contributor_id,kind,media_path,transcript,caption,summary,source_question_id,created_at",
        )
      ...
      sb
        .from("recall_events")
        .select(
          "id,family_id,status,keeper_results,gate,cue_text,cue_source,silence_reason,latency_ms,created_at",
        )
        .eq("family_id", familyId)
        .order("created_at", { ascending: false })
        .limit(8),
    ...
    const relativeList = relatives.data ?? [];
    const me = relativeList.find((r) => r.id === relativeId) ?? null;
    return NextResponse.json(
      {
        ...
        role,
        me,
        personNodes: (nodes.data ?? []).filter((n) => n.type === "person"),
        lovedOneConnected:
          members.data?.some((m) => m.role === "loved_one") ?? false,
        isOwner,
        email: user.email,
        wearer: wearer.data,
