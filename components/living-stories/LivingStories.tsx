...
function StoryRoom({ data, topic, refresh }: { data: FamilyData; topic: GraphNodeRow; refresh: () => Promise<void> }) {
  ...
  const ask = async () => {
    ...
    try {
      const response = await fetch(`/api/weaver/run?topic=${encodeURIComponent(topic.id)}`, { method: "POST" });
  ...
  const save = async (blob: Blob, mime: string) => {
    ...
    try {
      ...
      const response = await fetch("/api/memories/story", { method: "POST", body });
      const result = await response.json();
  ...
  return <div className={`${styles.stories} ${calm ? styles.calm : ""}`}>
    ...
    {me && data.role !== "loved_one" && <>
      ...
      <WeaverInbox me={me} relatives={data.relatives} questions={data.questions.filter((q) => q.gap_node_id === topic.id)} onAnswered={refresh} onOpenChange={setAnswering} />
    </>}
