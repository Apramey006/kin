...
Open **Stories** in the family navigation, or select a node in Connections and
choose **Hear the story**. Loved-one accounts can open **Listen to family stories**
from their recognition screen. Their player starts in Quiet view.
...
only the entities and relationships supported by the active contribution. Select
a map entity to follow its story. Quiet view removes the chapter list and map and
...
Apply `supabase/migrations/005_living_stories.sql` after migrations 001–004 to retain
...
Older recordings play in full. Deployments without migration 005 also continue to
work: an insert rejected specifically for the missing timing column is retried
without that column. Other insert errors are not retried. There is no automatic
retranscription or provider spending on playback.
...
the existing temporary-account setup and intercepts family/media/provider data.
Phone/Safari playback and real provider transcription still need device testing.

This implementation lives on the account-aware `refactor/kin-rust-handoff` branch.
It integrates with that branch's Connections map. It does not merge the divergent
Memory Atlas implementation from `main`.
