...
Follow README setup, including all four migrations and Supabase Auth callback
...
| 0:45–1:15 | Open camera, frame Nora or her photo, tap Who is this? | A name, relationship, and attributed family quote. Connections shows the sources. |
...
An optional backend check uses actual provider credits:

```bash
KIN_BASE_URL=http://localhost:3100 npx tsx scripts/check-live-memory.ts
```

It creates temporary accounts and a temporary family, generates fictional speech,
tests real transcription/extraction/storage, then removes its own test data.
...
- No recognition: two relatives must label the same person. Use a clear single face; don't lower the gate.
