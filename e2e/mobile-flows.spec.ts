...
test("photo preserves original bytes, requires explicit labels, and retries failed enrollment", async ({ page }) => {
  ...
  await expect(page.getByLabel("Person for face 1")).toHaveValue("");
  await page.getByLabel("Person for face 1").selectOption(nora.id);
  await page.getByRole("checkbox").check();
  await page.getByRole("button", { name: "Share this memory" }).click();
  await expect(page.locator('p[role="alert"]')).toContainText("enrollment did not finish");
  await expect(page.getByText("Memory shared. Selected faces enrolled.")).toHaveCount(0);
  await page.getByRole("button", { name: "Share this memory" }).click();
  await expect(page.getByText("Memory shared. Selected faces enrolled.")).toBeVisible();
...
test("stage terminal SILENT overrides stale speak gate and reset clears event", async ({ page }) => {
  let events: unknown[] = [{ id: "event", status: "silent", gate: { decision: "speak", V: 1, R: 1, A: 1, S: 1, X: 0, C: .95, threshold: .85 }, keeper_results: [], cue_text: "Old unsafe cue", silence_reason: "Grounding failed", latency_ms: 80 }];
  await page.route("**/rest/v1/recall_events?**", (route) => route.fulfill(json(events)));
  await page.route("**/api/admin/reset", (route) => { events = []; return route.fulfill(json({ ok: true })); });
