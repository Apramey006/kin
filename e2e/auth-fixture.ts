...
export async function signInDemo(page: Page, path = "/stage", admin = true, familyId = family, accountName = "Maya", familyData?: FamilyData) {
  ...
  await page.route("**/api/family", route=>route.fulfill(json(familyData ?? {familyId,relativeId:account.contributorId,role:account.role === "wearer" ? "loved_one" : "contributor",isOwner:admin,email:user.email,
