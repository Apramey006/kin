...
// Real browser camera capture; server inference and the backend response are
...
test("a new silent recall never replays the previous cue", async ({ page }) => {
  ...
  await page.route("**/api/recall", async (route) => {
    requests++;
    // Server inference receives image bytes; browser descriptors are forbidden.
    expect(route.request().postData()).toContain("snapshot");
