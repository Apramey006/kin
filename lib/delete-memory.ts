...
export async function deleteMemory(sb: SupabaseClient, familyId: string, contributorId: string, memoryId: string) {
  ...
  const [sources, nodes, edges, faces, questions, events] = await Promise.all([
    ...
    sb.from("face_embeddings").select("person_node_id, memory_id").eq("family_id", familyId),
    sb.from("weaver_questions").select("id, gap_node_id, answer_memory_id, evidence").eq("family_id", familyId),
    sb.from("recall_events").select("id, cue_source, gate, keeper_results").eq("family_id", familyId),
  ]);
  ...
  const eventIds = events.data!.filter((e) => e.cue_source?.memoryId === memoryId
  ...
  if (eventIds.length) {
    const updated = await sb.from("recall_events").update({ status: "silent", cue_text: null, cue_source: null,
